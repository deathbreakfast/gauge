...
async fn duplicate_named_super_user_group_does_not_grant_privilege_sad() {
    ...
    let created =
        gauge::generated::PermissionGroup::upsert(SUPER_USER_GROUP_ID, super_group, &system, valence::use_!(r"**Test:** Fixture **Permission Group** save for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
            .await
            .expect("upsert well-known super user group");

    let legit = lepton::generated::User::get("legit", &system, valence::use_!(r"**Test:** Fixture **User** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("query legit")
        .expect("legit exists");
    ...
    let fake_group =
        gauge::generated::PermissionGroup::upsert("fake_super_user_group", fake, &system, valence::use_!(r"**Test:** Fixture **Permission Group** save for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
            .await
            .expect("upsert fake super user group");

    let attacker = lepton::generated::User::get("attacker", &system, valence::use_!(r"**Test:** Fixture **User** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
...
async fn authenticated_user_cannot_mutate_domain_via_valence_sad() {
    ...
    let raw = valence::QueryCore::get_record_json(
        "permission_domain",
        &domain_id,
        &system,
    ...
    .expect("build domain");
    let direct_err = PermissionDomain::upsert(&domain_id, hijacked, &outsider_ctx, valence::use_!(r"**Test:** Fixture **Permission Domain** save for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
...
async fn owner_delete_group_happy_path() {
    ...
    let typed = gauge::generated::PermissionGroup::get(&group_id, &system, valence::use_!(r"**Test:** Fixture **Permission Group** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only.")).await;
    assert!(
